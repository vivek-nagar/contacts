...
import 'package:contact_app_by_vivek/db/contact_database.dart';
import 'package:contact_app_by_vivek/models/contactModal.dart';

class UsersNewProvid extends ChangeNotifier {
  ...
  void _handleAddUser(UserModel userModel) async {
    ...
    await ContactDataBaseProvid.db.insertUserData(userModel);
  ...
  void _handleUpdateUser(UserModel userModel) async {
    await ContactDataBaseProvid.db.updateUser(userModel);
    updateScreenData();
  }

  void _handleDeleteUser(int id) async {
    await ContactDataBaseProvid.db.deleteUser(id);
    updateScreenData();
  }

  void _handleSearchUser(String text) async {
    List<UserModel> user = await ContactDataBaseProvid.db.getUser();
    ...
    user.forEach((stud) {
      ...
      if ((st2.firstName.toLowerCase() + " " + st2.lastName.toLowerCase())
          ...
          st2.mobileNo.toLowerCase().contains(text.toLowerCase()) ||
          st2.email.toLowerCase().contains(text.toLowerCase()) ||
          st2.address.toLowerCase().contains(text.toLowerCase()) ||
  ...
  void updateScreenData() async {
    ...
    List<UserModel> users = await ContactDataBaseProvid.db.getUser();
